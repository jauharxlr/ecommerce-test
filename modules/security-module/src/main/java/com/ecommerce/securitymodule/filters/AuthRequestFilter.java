package com.ecommerce.securitymodule.filters;
...
public class AuthRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (appSecurityConfig.getIsBasicAuthEnabled() && StringUtils.isEmpty(username)) {
        ...
        if (null != authorizationHeader && authorizationHeader.contains("Basic")) {
            ...
            if (appSecurityConfig.getUsername().equals(basicUsername) && appSecurityConfig.getPassword().equals(basicPassword)) {
                ...
                if (StringUtils.hasText(username)) {
                    ...
                    userDetails = User.withUsername(basicUsername).password(basicPassword).authorities(Collections.emptyList()).accountExpired(false).accountLocked(false).credentialsExpired(false).disabled(false).build();
                }
                UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(userDetails, null, userDetails.getAuthorities());
                usernamePasswordAuthenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
                SecurityContextHolder.getContext().setAuthentication(usernamePasswordAuthenticationToken);
